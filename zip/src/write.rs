...
impl<W: Write + io::Seek> ZipWriter<W> {
    ...
    /// Write 5 bytes to a file that will produce no output when unzipped
    pub fn write_padding(&mut self) -> ZipResult<()> {
        match self.inner {
            #[cfg(any(
                feature = "deflate",
                feature = "deflate-miniz",
            ...
            GenericZipWriter::Deflater(ref mut w) => {
                w.get_mut().write_all(&[0, 0, 0, 255, 255])?;
            }
            _ => panic!(),
        }
        Ok(())
